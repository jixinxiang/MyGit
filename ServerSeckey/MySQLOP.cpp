...
bool MySQLOP::connectDB(string user, string passwd, string connstr)
{
	
	///< 连接的数据库（句柄、主机名、用户名、密码、数据库名、端口号、socket指针、标记）
	if (!mysql_real_connect(&mysql, connstr.c_str(), user.c_str(), passwd.c_str(), "ssl_test", 3306, nullptr, 0))
	{
...
void MySQLOP::updataKeyID(int keyID, int ikeysn)
{
	// 更新数据库
	string sql = "update keyID set ikeysn=" + to_string(keyID) + " " +"where ikeysn=" + to_string(ikeysn) + ";";
