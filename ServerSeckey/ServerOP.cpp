...
ServerOP::ServerOP(string json)
{
	...
	// 实例化一个连接oracle数据的对象
	mysql.connectDB(m_dbUser, m_dbPwd, m_dbSID);
...
string ServerOP::seckeyAgree(RequestMsg* reqMsg)
{
	...
	else
	{
		...
		node.seckeyID = mysql.getKeyID();	// 秘钥的ID
		info.seckeyID = node.seckeyID;
		node.status = 1;
		// 初始化node变量
		bool bl = mysql.writeSecKey(&node);
		if(bl)
		{
			// 成功
			int keysn = node.seckeyID + 1;
			mysql.updataKeyID(keysn, node.seckeyID);
			// 写共享内存
			m_shm->shmWrite(&node);
		}
		else
